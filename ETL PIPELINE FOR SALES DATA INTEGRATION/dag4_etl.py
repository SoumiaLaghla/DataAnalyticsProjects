...
def extract_delta_data(table_name, threshold_date, transactional_db_conn_id, data_warehouse_db_conn_id):
    ...
    is_empty = int(postgres_hook.get_pandas_df("select count(*) as e from fact_sales;")['e']) == 0
...
def transform_data(data):
    ...
    transformed_data = transformed_data[['customer_id', 'product_id', 'employee_id', 'quantity', 'discount', 'total_amount']]
    return transformed_data.to_json(orient='records')
    


def load_data(table_name, data, data_warehouse_db_conn_id):
    data = pd.DataFrame(json.loads(data))
    postgres_hook = PostgresHook(postgres_conn_id=data_warehouse_db_conn_id)
    sql = f"""
        ...
        VALUES (%s, %s, %s, %s, %s, %s);
    """
    postgres_hook.run(sql, autocommit=True, parameters=data.to_numpy().tolist())
